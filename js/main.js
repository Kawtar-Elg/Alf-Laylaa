...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeRoomFilters() {
    ...
    function applyFilters() {
      ...
      const selectedCategory = document.querySelector(
        'input[name="category_filter"]:checked'
      )?.value;
  ...
  function initializeAnimations() {
    ...
    document
      .querySelectorAll(".room-card, .service-card, .stat-card")
      .forEach(function (el) {
        el.style.opacity = "0";
    ...
    function updateTotalPrice() {
      if (checkinInputCalc.value && checkoutInputCalc.value) {
        ...
        if (nights > 0) {
          ...
          if (existingTotal) {
            existingTotal.textContent = `Total: ${LuxeHaven.formatCurrency(
              totalPrice
            )} (${nights} nights)`;
          } else {
            const totalElement = document.createElement("div");
            totalElement.className = "total-price mt-2 text-center";
            totalElement.innerHTML = `<strong>Total: ${LuxeHaven.formatCurrency(
              totalPrice
            )} (${nights} nights)</strong>`;
...
function cancelBooking(bookingId) {
  if (confirm("Are you sure you want to cancel this booking?")) {
    ...
    LuxeHaven.showNotification(
      "Booking cancellation functionality would be implemented here",
      "info"
    );
  }
}
